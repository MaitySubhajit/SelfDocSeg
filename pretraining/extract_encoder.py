...
if __name__ == "__main__":
    ...
    parser.add_argument('--weight_save_path', type=str, help='path to save extracted encoder weights')
    ...
    logs_root_dir = os.path.join(os.getcwd(), 'benchmark_logs') # args.logs_root

    weights = args.checkpoint                                                           # path to the saved checkpoint from pre-training
    save_path = args.weight_save_path                                                   # path of the extracted encoder weight file to be saved

    num_workers = 0 # args.num_workers                                                  # number of threads for dataloaders
    max_epochs = 800 # args.max_epochs                                                  # maximum epochs to train
    ...
    resume_ckpt = None # args.resume                                                    # path to the checkpoint for resuming training

    EETA_DEFAULT = 0.001 # args.eeta                                                    # small constant required for avoiding unfavourable calculations
    alpha = 1.0 # args.alpha                                                            # weightage for the mask loss (focal loss)
    beta = 1.0 # args.beta                                                              # weightage for the similatrity loss (nehgative cosine similarity loss)

    bin_threshold = 239 # args.bin_threshold                                            # threshold for binarization
    kernel_shape = 'rect' # args.kernel_shape                                           # kernel shape for erosion
    kernel_size = (3,3) # (args.kernel_size,args.kernel_size)                           # kernel size for erosion
    kernel_iter = 2 # args.kernel_iter                                                  # iterations for erosion

    # benchmark
    n_runs = 1 # args.n_runs                                                            # optional, increase to create multiple runs and report mean + std
    batch_size = 8 # args.batchsize                                                     # batch size, 8 is max for 24GB gpu using approximately 16GB

    learning_rate = 0.2 # args.learning_rate                                            # learning rate
    lr_decay = 5e-4 # args.lr_decay                                                     # weight decay for the learning rate
    wt_momentum = 0.99 # args.wt_momentum                                               # momentum for updating the weights in momentum branch

    # use a GPU if available
    gpus = 0 # torch.cuda.device_count() if torch.cuda.is_available() else 0            # forcing CPU operations

    # Set to True to enable Distributed Data Parallel training.
    distributed = False # args.distributed                                              # change to True to use more than one GPU

    # Set to True to enable Synchronized Batch Norm (requires distributed=True). 
    # If enabled the batch norm is calculated over all gpus, otherwise the batch
    # norm is only calculated from samples on the same gpu.
    sync_batchnorm = False # args.sync_batchnorm                                        # change to True to syncronize the batchnorm across GPUs

    # Set to True to gather features from all gpus before calculating 
    # the loss (requires distributed=True).
    # If enabled then the loss on every gpu is calculated with features from all 
    # gpus, otherwise only features from the same gpu are used.
    gather_distributed = False # args.gather_distributed                                # change to True to accumulate features from all GPUs before loss calculation
    ...
    class LayNet_SSL_dataset(torch.utils.data.Dataset):                                 # SSL Dataset class
        def __init__(self, dataset_path : str) -> None:
            ...
            self.img_path = os.path.join(self.dataset_path, 'train')
    ...
    module = SelfDocSeg(dataloader_kNN=dataloader_train_kNN, num_classes=classes)       # declaring the model
    module = module.to(device)
    module.load_state_dict(torch.load(weights, map_location=device)['state_dict'])      # loading checkpoint

    wts = module.backbone.state_dict()                                                  # extracting the weight of the loaded backbone
    resnet = torchvision.models.resnet50(weights=None).to(device)                       # declaring a new encoder to reload the weights
    resnet_seq = nn.Sequential(*list(resnet.children())[:-1]).to(device)
    resnet_seq.load_state_dict(wts)                                                     # loading weights to the new encoder

    torch.save(resnet.state_dict(), save_path)                                          # saving weights from new encoder
    print('Encoder extracted in Torchvision Resnet structure!')